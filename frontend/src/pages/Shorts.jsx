...
export default function Shorts({ nav, onSelect }) {
  ...
  if (videos.length === 0) return (
    <div style={{ minHeight: "100vh", background: "#000" }}>
      ...
      <div style={{
        ...
        <button onClick={nav.onUpload} style={{
...
const ShortItem = forwardRef(function ShortItem({ video, isActive, onVisible }, ref) {
  ...
  useEffect(() => {
    ...
    if (!vid) return
    const onLoop = () => setTimeout(() => fireTopComments(), 800)
    const onFirstPlay = () => {
  ...
  return (
    <div ref={ref} style={{
      ...
      <video
        ...
        loop playsInline
        onClick={() => videoElRef.current.paused 
            ? videoElRef.current.play() 
            : videoElRef.current.pause()
        }

        style={{ height: "100%", maxWidth: "100%", objectFit: "contain", cursor: "pointer" }}
      />
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: "#ccc", marginBottom: 10 }}>
          ...
          {video.creator_tier === "new" && (
            <span style={{ marginLeft: 8, color: "#ffcc44", fontSize: 11 }}>New Creator</span>
          )}
        </div>
        {topComments[0] && (
          <div style={{
            ...
            <span style={{ fontSize: 11, color: "#ffcc44" }}>Top</span>
            {topComments[0].text}
            <span style={{ fontSize: 11, color: "#666" }}>· {topComments[0].upvotes}</span>
          </div>
        )}
      </div>
      ...
      <div style={{
        ...
        <ActionBtn icon={liked ? "❤️" : "🤍"} label={likes} onClick={like} />
      ...
      {showComments && (
        <div style={{
          ...
          <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 14, color: "#888" }}>
            Comments
          </div>
          {topComments.map(c => (
            <div key={c.id} style={{
              ...
              <span style={{ fontSize: 11, color: "#444" }}>{c.upvotes}</span>
            </div>
          ))}
          <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
            <input
              ...
              style={{
                flex: 1, padding: "9px 14px", background: "#111",
                border: "1px solid #222", borderRadius: 20,
                color: "#fff", fontSize: 13, outline: "none"
              }}
            />
            <button onClick={sendComment} style={{
              padding: "9px 16px", background: "#fff", color: "#000",
